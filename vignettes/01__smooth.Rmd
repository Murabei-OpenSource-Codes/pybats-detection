...
```{r, echo=FALSE, fig.dim=c(16, 9), out.width = "90%", "plots for smooth simulated example", fig.cap="Mean response for Filtered and Smoothed predictive distributions with $95\\%$ credibility intervals."}
...
```{r, message = FALSE, warnings = FALSE, echo=FALSE, fig.dim=c(16,9), out.width="90%", fig.cap="Mean response for Filtered and Smoothed posterior distributions for each model component with $95\\%$ credibility intervals."}
...
p1 = posteriori_filter_df %>% filter(parameter == 'Intercept') %>% 
  posterior_plot + ggtitle('Filter', subtitle = 'Level')
p2 = posteriori_filter_df %>% filter(parameter == 'Local slope') %>% 
  posterior_plot + ggtitle('Filter', subtitle = 'Growth') + 
  ylim(-2.0, 2.0)
p3 = posteriori_smooth_df %>% filter(parameter == 'Intercept') %>% 
  posterior_plot + ggtitle('Smooth', subtitle = 'Level') 
p4 = posteriori_smooth_df %>% filter(parameter == 'Local slope') %>% 
  posterior_plot + ggtitle('Smooth', subtitle = 'Growth') + 
  ylim(-2.0, 2.0)
...
```{r, echo=FALSE, fig.dim=c(16, 9), out.width="90%", "plots for airpassangers example", fig.cap="Mean response for Filtered and Smoothed predictive distributions with $95\\%$ credibility intervals."}
predictive_filter_df = py$predictive_filter_df 
predictive_smooth_df = py$predictive_smooth_df 
posteriori_smooth_df = py$posteriori_smooth_df 
posteriori_filter_df = py$posteriori_filter_df 
...
```{r, echo=FALSE, fig.dim = c(16, 9), out.width="90%", "components for airpassangers example", fig.cap="Mean response for Filtered and Smoothed posterior distributions for each model component with $95\\%$ credibility intervals."}
date_posterior_plot = function(data){
  data %>%
    ...
    scale_x_date(breaks = scales::pretty_breaks(5), date_labels = "%b/%Y") +
    theme(axis.text=element_text(size=15), 
          axis.title=element_text(size=15)) + 
    xlab('Month/Year')
}

p1 = posteriori_smooth_df %>%
  filter(parameter == 'Intercept') %>% 
  date_posterior_plot +
  ggtitle('Level')
p3 = posteriori_smooth_df %>%
  filter(parameter == 'Local Slope') %>% 
  date_posterior_plot +
  ggtitle('Growth')
p4 = posteriori_smooth_df %>%
  filter(parameter == 'theta_7') %>% 
  date_posterior_plot +
  ggtitle('Seasonality')
