...
A brief introduction of the Smooth class in a simulated example.The time series 
$\mathbf{Y} = (y_1, \dots, y_n)$ was generated using the 
$RandomDLM$ class, which has as arguments (n, V, W) the number of observations, observational variance and state vector variance. The $.level$ method allows you 
to define the starting level and regime change points.
...
mod = dlm(a, R, ntrend=2, deltrend=1)
...
```{python}
def shortcut_run(deltrend: np.float64):
  """short cut for test smoothing with by deltrend."""
  a = np.array([100, 0])
  R = np.eye(2)
  np.fill_diagonal(R, val=1)
  mod = dlm(a, R, ntrend=2, deltrend=deltrend)
  
  # Fit with monitoring
  smooth = Smoothing(mod=mod)
  results_dict = smooth.fit(y=y).get('smooth')
  results_dict.get('predictive').loc[:, 'delta'] = deltrend
  results_dict.get('posterior').loc[:, 'delta'] = deltrend
  results_dict.get('predictive').loc[:, 'real'] = y
  
  return results_dict

deltrends = [0.95, 1]
predictive_smooth_df = pd.concat([shortcut_run(i).get('predictive') for i in deltrends])
posteriori_smooth_df = pd.concat([shortcut_run(i).get('posterior') for i in deltrends])
```
...
p1 = predictive_smooth_df %>% filter(delta == 0.95) %>%
  ggplot(aes(x = t)) + 
  geom_point(aes(y = real)) + 
  geom_line(aes(y = fk), colour = 'dodgerblue4') + ylab(' ') + 
  geom_ribbon(aes(ymin=ci_lower, ymax=ci_upper), 
              alpha=0.25, colour = 'grey40') + 
  scale_y_continuous(breaks = scales::pretty_breaks(10)) +
  ggtitle('deltrend = 0.95')

p2 = predictive_smooth_df %>% filter(delta == 1) %>%
  ggplot(aes(x = t)) + 
  geom_point(aes(y = real)) + 
  geom_line(aes(y = fk), colour = 'dodgerblue4') + ylab(' ') + 
  geom_ribbon(aes(ymin=ci_lower, ymax=ci_upper), 
...
p1 = posteriori_smooth_df %>% filter(parameter == 'theta_1', delta == .95) %>% 
  posterior_plot + ggtitle('Component: Level', subtitle = 'deltrend = 0.95')
p2 = posteriori_smooth_df %>% filter(parameter == 'theta_2', delta == .95) %>% 
  posterior_plot + ggtitle('Component: Growth', subtitle = 'deltrend = 0.95') + 
  ylim(-.8, .8)
p3 = posteriori_smooth_df %>% filter(parameter == 'theta_1', delta == 1) %>% 
  posterior_plot + ggtitle('Component: Level', subtitle = 'deltrend = 1') 
p4 = posteriori_smooth_df %>% filter(parameter == 'theta_2', delta == 1) %>% 
  posterior_plot + ggtitle('Component: Growth', subtitle = 'deltrend = 1') + 
  ylim(-.8, .8)
